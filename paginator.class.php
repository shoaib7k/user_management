...
class Paginator
{
...

	{
		...
		if ($this->num_pages > 1) {
			...
			$this->return .= (($this->current_page < $this->num_pages and $this->items_total >= 10) and ($_GET['page'] != 'All') and $this->current_page > 0) ? "<li class='page-item'><a class=\"page-link\" href=\"$_SERVER[PHP_SELF]?page=$next_page&ipp=$this->items_per_page$this->querystring\">" . $_next . "</a></li>\n" : "<li class='page-item'><a href=\"javascript:;\" class=\"page-link disabled\" href=\"javascript:;\" tabindex=\"-1\">Next</a></li>\n";
		...
		$this->limit = (isset($_GET['ipp']) && $_GET['ipp'] == 'All') ? "" : " LIMIT $this->items_per_page OFFSET $this->low";
	}
...

	{
		...
		return "<div class='col-sm-5 float-sm-right'><div class='form-row mt-2 text-right'><div class='col'><span class=\"text-muted\">".$_rows.":</span> <select class=\"border rounded text-muted\" onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select></div>\n";
	}
...

	{
		...
		return "<div class='col'><span class=\"text-muted\">".$_page.":</span> <select class=\"border rounded text-muted\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&ipp=$this->items_per_page$this->querystring';return false\">$option</select></div><div class='col'><strong class='text-danger'>$_total: " . $this->items_total . "</strong></div></div></div></div>\n";
	}
